...
from compgcn_utils import ccorr
...
class CompGraphConv(nn.Module):
    ...
    def forward(self, g, n_in_feats, r_feats):
        ...
        with g.local_scope():
            ...
            new_comp_h = th.zeros(comp_h.shape[0], self.out_dim, dtype=comp_h_O.dtype).to(
                comp_h.device
            )
            new_comp_h[out_edges_idx] = comp_h_O # <---- THERE WAS A COMPLAINT ABOUT FLOAT32-FLOAT16 MISMATCH, 
            new_comp_h[in_edges_idx] = comp_h_I  #       I HAD TO CHANGE THE DTYPE OF new_comp_h
